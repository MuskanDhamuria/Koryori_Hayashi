...
function revenueForOrders(orders) {
    return roundNumber(orders.reduce((sum, order) => sum + Number(order.totalAmount), 0));
}
function orderMarginPercent(orders) {
    const revenue = orders.reduce((sum, order) => sum +
        order.orderItems.reduce((orderSum, item) => orderSum + Number(item.lineTotal), 0), 0);
    if (revenue === 0) {
        return 0;
    }
    const cost = orders.reduce((sum, order) => sum +
        order.orderItems.reduce((orderSum, item) => orderSum + Number(item.menuItem.cost) * Number(item.quantity), 0), 0);
...
function averageOrderValue(orders) {
    ...
    return roundNumber(orders.reduce((sum, order) => sum + Number(order.totalAmount), 0) / orders.length);
}
...
export const analyticsRoutes = async (app) => {
    app.get("/dashboard", { preHandler: app.requireStaff }, async () => {
        const [orders, inventoryItems, menuItems, staffCount, activeTables] = await Promise.all([
        ...
        const previousWeekOrders = orders.filter((order) => order.orderedAt >= twoWeeksAgo && order.orderedAt < oneWeekAgo);
        ...
        const ordersGrowth = previousWeekOrdersCount === 0
            ? 0
            : roundNumber(((weekOrdersCount - previousWeekOrdersCount) / previousWeekOrdersCount) * 100);
        ...
        const peakHourEntry = Array.from(hourlyRevenue.entries()).sort((a, b) => b[1] - a[1])[0] ?? null;
        const peakHour = peakHourEntry?.[0] ?? 12;
        const peakHourRevenue = roundNumber(peakHourEntry?.[1] ?? 0);
        const uniqueCustomersThisWeek = new Set(currentWeekOrders.map((order) => order.userId ?? order.id)).size;
        ...
        const criticalInventory = inventoryWithCoverage.filter((item) => item.stockOnHand <= Math.max(1, Math.floor(item.reorderPoint / 2)));
        const warningInventory = inventoryWithCoverage.filter((item) => item.stockOnHand > Math.max(1, Math.floor(item.reorderPoint / 2)) &&
            item.stockOnHand <= item.reorderPoint);
        const overstockInventory = inventoryWithCoverage.filter((item) => item.stockOnHand >= item.reorderPoint * 3);
        const marginBuckets = menuItems.reduce((accumulator, item) => {
        ...
        const topMarginOpportunity = currentWeekOrders
            .flatMap((order) => order.orderItems)
            .map((item) => ({
            name: item.menuItem.name,
            revenue: Number(item.lineTotal),
            marginPercent: ((Number(item.menuItem.price) - Number(item.menuItem.cost)) /
        ...
        return {
            ...
            inventorySummary: {
                ...
                healthyCount: inventoryItems.length - criticalInventory.length - warningInventory.length,
                overstockCount: overstockInventory.length,
                averageCoverageDays: inventoryWithCoverage.length === 0
                    ? 0
                    : roundNumber(inventoryWithCoverage.reduce((sum, item) => sum + item.coverageDays, 0) / inventoryWithCoverage.length),
            ...
            marginSummary: {
                ...
                recommendation: topMarginOpportunity === null
