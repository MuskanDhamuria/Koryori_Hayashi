...
type OrderContextDb = Pick<typeof prisma, "restaurantTable" | "menuItem" | "user">;
      ...
      return {
        pricing: {
          ...
          pointsEarned: Math.floor(
            context.pricing.totalAmount * getTierMultiplier(context.currentTier),
          ),
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (customer) {
          const earnedPoints = Math.floor(
            context.pricing.totalAmount * getTierMultiplier(currentTier),
          );
